...
public class ConsumirAPI : MonoBehaviour
{
    ...
    IEnumerator PostData(string jsonDato)
    {
        ...
        else
        {
            ...
            if (loginResponse.Mensaje != "El usuario y/o contraseña son inválidos")
            {
                ...
                form.AddField("id_usuario", int.Parse(loginResponse.Datos.Identificacion));

                string url_base = ConfiguracionGeneral.configuracionDefault.url;
                // Enviar la solicitud POST
                using (UnityWebRequest www = UnityWebRequest.Post(url_base + url, form))
                {
                ...
                if (loginResponse.Datos.TipoDeUsuario == "Estudiante")
                {
                    ...
                    //PersonajeBD.instance.tipo_usuario = 2;
                }
            ...
            {
                txtError.text = "Usuario no válido" +  solicitud.error + "\n" + "Codigo de respuesta: " + solicitud.responseCode + "\n" + "URL: " + solicitud.url;
                imgCarga.SetActive(false);
                gmError.SetActive(true);
                inputPassword.text = "";
			}
    ...
    IEnumerator PostDataFake()
    {
        // Creación del formulario
        WWWForm form = new WWWForm();
        form.AddField("id_usuario", int.Parse(inputUsuario.text));
        ...
        // Guardamos la identificacion del usuario convertida a entero
        managerBD.id_usuario = int.Parse(inputUsuario.text);
